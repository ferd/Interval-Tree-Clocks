...
norm_ev({N, M, M}) when is_integer(M) ->
    N + M;
norm_ev({N, L, R}) ->
    M = min(base(L), base(R)),
...
ence({0, L, 0}) ->
    <<0:1, 1:2, (ence(L))/bits>>;
ence({0, L, R}) ->
    <<0:1, 2:2, (ence(L))/bits, (ence(R))/bits>>;
ence({N, 0, R}) ->
    <<0:1, 3:2, 0:1, 0:1, (ence(N))/bits, (ence(R))/bits>>;
ence({N, L, 0}) ->
    <<0:1, 3:2, 0:1, 1:1, (ence(N))/bits, (ence(L))/bits>>;
ence({N, L, R}) ->
    <<0:1, 3:2, 1:1, (ence(N))/bits, (ence(L))/bits, (ence(R))/bits>>;
ence(N) ->
    encn(N, 2).
...
encn(N, B) when N < (1 bsl B) ->
    <<((1 bsl B) - 2):B, N:B>>;
encn(N, B) ->
    encn(N - (1 bsl B), B + 1).

decn(<<0:1, R/bits>>, B, Acc) ->
    <<N:B, R1/bits>> = R, {N+Acc, R1};
